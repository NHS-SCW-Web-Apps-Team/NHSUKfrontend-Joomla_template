...
<div id="lists" class="toolbar-content">
	<small class="text-muted">
		<span><?php echo JText::_('COM_TJLMS_LIST_HELP_TEXT'); ?></span>
	</small>
	<div class="clearfix"></div>
	<?php
		$dispatcher = JDispatcher::getInstance();
		JPluginHelper::importPlugin('content');
		$result =$dispatcher->trigger('showLists',array('com_tjlms.lesson',$lesson_data->id,$lesson_data->title));
		if(!empty($result))
		echo $result[0];
	?>
</div>
<div id="notes" class="toolbar-content">
	<small class="text-muted">
		<span><?php echo JText::_('COM_TJLMS_NOTE_HELP_TEXT'); ?></span>
	</small>
	<div class="clearfix"></div>
		<?php
		$dispatcher = JDispatcher::getInstance();
		JPluginHelper::importPlugin('content');
		$result =$dispatcher->trigger('showNotes',array('com_tjlms.lesson',$lesson_data->id,$lesson_data->title));
		if(!empty($result))
		echo $result[0];
	?>
</div>
<div id="comments" class="toolbar-content">
	<?php
		$dispatcher = JDispatcher::getInstance();
		JPluginHelper::importPlugin('content');
		$result =$dispatcher->trigger('showComments',array('com_tjlms.lesson',$lesson_data->id,$lesson_data->title));
		if(!empty($result))
		echo $result[0];
	?>
</div>
...
<?php if ($this->allowAssocFiles == 1): ?>
	...
	<div class="associatefiles toolbar-content" id="associatefiles">
		<small class="text-muted">
			<span><?php echo JText::_('COM_TJLMS_ASSOC_FILES_HELP_TEXT'); ?></span>
		</small>
		<div class="clearfix"></div>
		...
		<?php foreach($this->lesson_typedata->associateFiles as $assocFile): ?>
			...
			<div class="assocfilecontainer">
				...
				<div class="span1">
					...
					?>
					<a target="_blank" href="<?php echo $downloadUrl; ?>" title="<?php echo JText::_('COM_TJLMS_ASSOC_FILES_DOWNLOAD'); ?>"><i class="fa fa-download"></i></a>
				</div>
			</div>
			<div class="clearfix"></div>
				<hr class="hr hr-condensed">
	...
	<?php else: ?>
		...
		<div class="alert alert-warning">
			<?php echo JText::_('COM_TJLMS_NO_ASSOCIATE_FILES');?>
		</div>
