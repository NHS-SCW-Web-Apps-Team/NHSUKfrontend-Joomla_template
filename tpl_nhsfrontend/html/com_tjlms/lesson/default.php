...
if ($this->usercanAccess['access'] == 0)
{
	?>
		<div class="alert alert-danger">
			...
			<span><?php echo JText::sprintf('COM_TJLMS_LESSON_CLICK_COURSE_LINK', $this->returnUrl);?></span>
		</div>
	<?php
	return;
}
...
$jLikeInteractions = $dispatcher->trigger('getLessonInteractions', array($this->lesson_id));
	...
	<div class="container-fluid">
		...
		<?php elseif($this->mode == 'preview' && $close !== 0): ?>
			<div class="navbar-fixed-top" id="admin-close-button">
				<button type="button" class="close" onclick="tjLmsCommon.closePopup();"; data-dismiss="modal" aria-hidden="true"><!--<i class="fa fa-close"></i>-->Save and close</button>
			</div>
		<?php endif; ?>
					...
					<span class="help-block"><?php echo JText::_('COM_TJLMS_INCOMPLETE_LAST_ATTEMPT_MSG'); ?>
						...
						if($lesson_data->format!='scorm' && $lesson_data->format!='tjscorm' && $lesson_data->format!='textmedia' && $lesson_data->format!='externaltool')
						{
							$lang_constant_toshow	=	"COM_TJLMS_INCOMPLETE_LAST_ATTEMPT_STATUS_".$lesson_data->format;
							 echo JText::sprintf($lang_constant_toshow, $this->lastattempttracking_data->currentPositionFormat, $lesson_data->title, $this->lastattempttracking_data->totalContentFormat);
						}
					...
					<div class="container-fluid">
						<div class="row">
							<div class="col-xs-6 col-sm-6 text-right">
								<input type="button" name="new" value="<?php echo JText::_('COM_TJLMS_NEW_ATTEMPT') ?>" class="btn btn-default btn-medium" onclick="askforaction('start','<?php echo $lesson_data->id; ?>','<?php echo $lesson_url?>','<?php echo $this->attempt; ?>','<?php echo $lesson_data->format; ?>');">
							</div>
							<div class="col-xs-6 col-sm-6 text-left">
								<input type="button" id="old" name="old" value="<?php echo JText::_('COM_TJLMS_CONTINUE_OLD') ?>" class="btn btn-default btn-medium" onclick="askforaction('resume','<?php echo $lesson_data->id; ?>','<?php echo $lesson_url?>','<?php echo $this->attempt; ?>','<?php echo $lesson_data->format; ?>');">
							</div>
						</div>
					</div>
...
<script>
<?php
	JText::script('COM_TJLMS_LESSON_CONFIRM_BOX');
?>
